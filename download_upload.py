...
def compress_image_if_necessary(file_path):
    ...
    if megapixels >= 20:
        print(
            f"Image {file_path} is 20MP or higher. Compressing further to a maximum of 15MP..."
        )
        scale_factor = (15 / megapixels) ** 0.5
        img = img.resize(
            (int(width * scale_factor), int(height * scale_factor)), Image.ANTIALIAS
        )
        img.save(file_path, "JPEG", quality=90)
    ...
    for i, album_url in enumerate(album_main_links):
        ...
        while os.path.exists(output_folder):
            output_folder = os.path.join(
                "JIN-Photos", folder_name + f"({folder_index})"
            )
            folder_index += 1
